...
imageMode("papar.png",ball.position.x,ball.position.y,radius,radius);
	// use image() command to add dustbin image in the canvas.
	imageMode("dustbin.png",1185, 570, 200,200);
...
function keyPressed() {
  	if (keyCode === UP_ARROW) {
		...
		Matter.Body.applyForce("papar.png",ball.position,{x:85,y:-85});
